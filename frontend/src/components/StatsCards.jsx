...
export default function StatsCards() {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-6">
      {stats.map((stat, i) => (
        <div key={i} className={`bg-white dark:bg-slate-800 rounded-xl p-4 lg:p-5 shadow-sm border-t-4 ${stat.border} transition-colors duration-300`}>
          <div className={`text-xl lg:text-2xl mb-2 ${stat.color}`}>{stat.icon}</div>
          <div className="flex items-baseline gap-1 lg:gap-2 flex-wrap">
            <span className="text-2xl lg:text-3xl font-bold text-slate-800 dark:text-white transition-colors duration-300">
              {stat.count}
            </span>
            <span className="text-xs lg:text-sm font-semibold text-slate-700 dark:text-slate-300 transition-colors duration-300">
              {stat.label}
            </span>
          </div>
